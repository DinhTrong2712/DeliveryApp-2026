...
function ChangeCell({ oldVal, newVal }: { oldVal: string | null; newVal: string | null }) {
  if (!oldVal && !newVal) return <span className="text-gray-400 dark:text-gray-500">—</span>
  if (oldVal && newVal) {
    return (
      <span className="text-sm text-gray-700 dark:text-gray-300">
        <span className="text-gray-400 dark:text-gray-500">{oldVal}</span>
        <span className="text-gray-400 dark:text-gray-500 mx-1">→</span>
        <span className="font-medium">{newVal}</span>
      </span>
    )
  }
  return <span className="text-sm text-gray-700 dark:text-gray-300">{newVal ?? oldVal}</span>
}
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Lịch sử thao tác</h1>
          <p className="text-sm text-gray-500 dark:text-gray-500 mt-0.5">{total.toLocaleString('vi-VN')} bản ghi</p>
        </div>
        <button
              ...
              {ACTION_OPTIONS.map(opt => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors ${
                    actionFilter === opt.value ? 'bg-orange-50 dark:bg-orange-500/10 font-semibold text-orange-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-300'
                  }`}
                >
                  {opt.label}
                </button>
        ...
        <table className="w-full text-sm min-w-[900px]">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
              ...
              <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Người thực hiện</th>
              ...
              <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Thay đổi</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 dark:text-gray-400">Ghi chú</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="text-center py-16 text-gray-400 dark:text-gray-500">Không có bản ghi</td>
              </tr>
            ) : logs.map((log, i) => (
              <tr
                key={log.id}
              ...
              >
                ...
                <td className="px-4 py-3">
                  {log.performedBy === 'Hệ thống' ? (
                    <span className="italic text-gray-400 dark:text-gray-500 text-sm">Hệ thống</span>
                  ) : (
                    <span className="font-bold text-gray-900 dark:text-gray-100 text-sm">{log.performedBy}</span>
                  )}
                </td>
                <td className="px-4 py-3">
                  {log.orderCode ? (
                    <span className="font-mono text-sm italic text-gray-500 dark:text-gray-500">{log.orderCode}</span>
                  ) : (

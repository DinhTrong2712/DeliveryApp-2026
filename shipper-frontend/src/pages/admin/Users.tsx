...
const ROLE_BADGE: Record<string, string> = {
  Admin: 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-700',
    ...
    <AdminLayout>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
          <h1 className="font-semibold text-gray-900 dark:text-gray-100 text-lg">Người dùng</h1>
          <button
            onClick={openAdd}
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead>
            <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 text-left">
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Họ tên</th>
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Tên đăng nhập</th>
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Vai trò</th>
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Tên trên file Excel</th>
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Trạng thái</th>
              <th className="px-6 py-3 font-medium text-gray-500 dark:text-gray-500">Hành động</th>
            </tr>
          </thead>
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                ...
                <td className="px-6 py-4 text-gray-500 dark:text-gray-500">{u.username}</td>
                <td className="px-6 py-4">
                  <span className={`inline-block px-2.5 py-0.5 rounded-md text-xs font-medium ${ROLE_BADGE[u.role] ?? 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>
                    {ROLES[u.role] ?? u.role}
                  </span>
                </td>
                <td className="px-6 py-4 text-gray-500 dark:text-gray-500">{u.xlsxName ?? '–'}</td>
                <td className="px-6 py-4">
            ...
            {users.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-gray-400 dark:text-gray-500">Chưa có người dùng</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="font-semibold text-gray-900 dark:text-gray-100">{editUser ? 'Sửa tài khoản' : 'Thêm tài khoản'}</h2>
              <button onClick={handleClose} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {!editUser && (
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Mật khẩu {editUser ? <span className="text-gray-400 dark:text-gray-500 font-normal">(để trống nếu không đổi)</span> : '*'}
                </label>
                <input
                  type="password"
                  value={form.password}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tên trên file Excel</label>
                <input
                  value={form.xlsxName}

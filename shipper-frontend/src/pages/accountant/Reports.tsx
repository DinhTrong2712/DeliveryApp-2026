...
const dash = (n: number | undefined) =>
  !n ? <span className="text-gray-400">—</span> : <>{fmt(n)}</>
const dashCount = (n: number | undefined) =>
  !n ? <span className="text-gray-400">—</span> : <>{n}</>
...
export default function Reports() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      valueCls: (data?.unpaidAmount ?? 0) > 0 ? 'text-red-600' : 'text-gray-900',
      icon: (
    ...
    },
    {
      ...
      valueCls: (data?.scheduledAmount ?? 0) > 0 ? 'text-orange-600' : 'text-gray-900',
      icon: (
        <svg className="w-5 h-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <AccountantLayout>
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl font-bold text-gray-900">Báo cáo ngày</h1>
        ...
        <>
          ...
          <div className="flex items-center gap-4 text-sm mb-5 px-1">
            <span className="text-gray-600">
              Tổng đã thu: <span className="font-semibold text-gray-900">{fmt(totalCollected)}</span>
            </span>
            <span className="text-gray-400">·</span>
            <span className="text-gray-600">
              Tỷ lệ: <span className="font-semibold text-gray-900">{collectionRate}%</span>
            </span>
            <span className="text-gray-400">·</span>
            <span className="text-gray-600">
              Tổng công nợ:{' '}
              <span className={`font-semibold ${(data.unpaidAmount + data.scheduledAmount) > 0 ? 'text-red-600' : 'text-gray-900'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-700">Chi tiết theo nhân viên</h2>
            </div>
            ...
            {data.byShipper.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50 text-gray-600 text-xs">
                  ...
                  <tbody>
                    {data.byShipper.map((s, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-3 py-3 text-center text-gray-700">{s.totalOrders}</td>
                          <td className="px-3 py-3 text-right text-gray-900">{fmt(s.totalAmount)}</td>
                          ...
                          <td className="px-3 py-3 text-right text-red-500 font-medium">
                            {remaining > 0 ? fmt(remaining) : <span className="text-gray-400">—</span>}
                          </td>
                    ...
                    {totals && (
                      <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold text-sm">
                        ...
                        <td className="px-3 py-3 text-center text-gray-900">{totals.orders}</td>
                        <td className="px-3 py-3 text-right text-gray-900">{fmt(totals.totalAmount)}</td>
                        ...
                        <td className="px-3 py-3 text-center text-gray-900">
                          {totals.waitingCK > 0 ? totals.waitingCK : <span className="text-gray-400">—</span>}
                        </td>
                        ...
                        <td className="px-3 py-3 text-right text-gray-900">
                          {totals.scheduled > 0 ? fmt(totals.scheduled) : <span className="text-gray-400">{fmt(0)}</span>}
                        </td>
                        <td />
                      </tr>
                    )}
              ...
              <div className="py-8 text-center text-gray-400 text-sm">Không có dữ liệu cho ngày này</div>
            )}
          </div>
        </>
      )}

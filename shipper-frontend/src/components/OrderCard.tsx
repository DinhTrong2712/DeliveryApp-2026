...
export function OrderCard({ order, basePath }: Props) {
  ...
  return (
    <Link to={`${basePath}/${order.id}`}>
      <div
        className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 border-l-4 ${borderColor} p-4 mb-3 active:bg-gray-50 transition-colors flex items-center gap-3`}
      >
        <div className="flex-1 min-w-0">
          {/* Order code + status badge */}
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            <span className="font-bold text-gray-900 dark:text-gray-100 text-sm">{order.orderCode}</span>
            <StatusBadge status={order.status} />
          </div>
          <p className="text-gray-500 dark:text-gray-500 text-sm truncate">{order.customerName}</p>

          {/* Amounts */}
          <div className="flex items-center gap-4 mt-2">
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500">Tổng tiền</p>
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">{formatVND(order.amount)}</p>
            </div>
            ...
            {isPaid && (
              <div>
                <p className="text-xs text-gray-400 dark:text-gray-500">Thanh toán</p>
